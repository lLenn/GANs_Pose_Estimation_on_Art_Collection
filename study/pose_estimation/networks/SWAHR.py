...
class SWAHR():
    def __init__(self, config, phase="train"):
        ...
        if config.TEST.MODEL_FILE:
            self.loadState(config.TEST.MODEL_FILE, True)
    ...
    def validate(self, gpuIds, dataset, indices, logger):
        ...
        self.model = torch.nn.DataParallel(self.model, device_ids=gpuIds).cuda()
        self.model.eval()
        ...
        for i, (images, annotations) in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                for idx, s in enumerate(sorted(self.config.TEST.SCALE_FACTOR, reverse=True)):
                    ...
                    outputs, heatmaps, tags = get_multi_stage_outputs(
                        self.config, self.model, image_resized, self.config.TEST.FLIP_TEST,
                ...
                if visual:
                    ...
                    cv2.imwrite(".output/test_data/{}.jpg".format(int(annotations[0]["image_id"])), test_data)
            ...
            file_name = dataset.humanArt.loadImages(img_id)[0]["file_name"]
    ...
    def print(self):
        print("Scale- and weight-adaptive heatmap regression")